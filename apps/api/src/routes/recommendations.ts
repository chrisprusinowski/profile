...
function canEditRecommendation(
  userRole: 'admin' | 'executive' | 'manager',
  recStatus: string,
  cycleStatus: string
) {
...
async function getScopedRecommendations(
  cycleId: number,
  ...
  async (req: AuthenticatedRequest, res, next) => {
    ...
    try {
      ...
      const employeeResult = await pool.query(
        'SELECT id, salary::float AS salary, hire_date::text AS hire_date FROM employees WHERE id = $1',
        [employeeId]
      );
      ...
      if (!canEditRecommendation(req.user!.role, currentStatus, cycle.status)) {
        res
          .status(409)
          .json({
            error: 'Recommendation is read-only in current workflow state'
          });
        return;
      }
      ...
      if (ineligible && !cycle.allow_eligibility_override) {
        res
          ...
          .json({
            error:
              'Employee is ineligible for this cycle based on current settings'
          });
        return;
      }
