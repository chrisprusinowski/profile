...
// Returns { [employeeId]: { meritPct, rating, notes, status, updatedAt } }
...
recommendationsRouter.put('/:employeeId', async (req, res) => {
...
// POST /api/v1/recommendations/submit-all
// Transitions all Draft recs to Submitted for the current cycle
recommendationsRouter.post('/submit-all', async (_req, res) => {
  const cycleId = await getCurrentCycleId();
  if (!cycleId) {
    res.status(400).json({ error: 'No active cycle' });
    return;
